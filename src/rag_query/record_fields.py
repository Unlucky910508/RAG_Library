...
A copy of src/common/record_fields.py, kept here so this directory can be
handed over on its own. The pipeline's copy renders the text that gets
embedded; this one renders the text a hit is answered with. They have to
agree on what a field looks like, so a change to one belongs in the other.
